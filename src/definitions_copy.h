...
//TF1 * f_trig_ec =(TF1 *)ftrigger->Get("f_trig_ec");
//std::vector<TF1*> fTrigEff_; 
//fTrigEff_.push_back((TF1*)ftrigger->Get("f_trig_eb"));
//fTrigEff_.push_back((TF1*)ftrigger->Get("f_trig_ec"));
/*
template<typename ntupleType> double Trigger_weights(ntupleType* ntuple){
 if( ntuple->Photons_isEB->at(0) ) {
       return  fTrigEff_[0].Eval(max(double(205),ntuple->Photons.at(0).Pt()));  // hard-wired cutoff
     }
 else {
       return  fTrigEff_[1].Eval(max(double(205),ntuple->Photons.at(0).Pt()));  // hard-wired cutoff
     }
